...
export default function HipDiagnosisModal({ isOpen, onClose, textareaRef }) {
  const diagnoses = ["Артроз 1 ст", "Артроз 2 ст", "Артроз 3 ст", "Артроз 4 ст", "Вывих", "Коксартроз", "Асептический некроз"];
  ...
  const zones = [
    { key: "right", name: "Правый ТБС", position: { top: "200px", left: "35%" } },
    { key: "left", name: "Левый ТБС", position: { top: "200px", left: "65%" } },
  ];
  ...
  const generateDiagnosisDescription = () => {
    ...
    if (!leftDiagnosis && !rightDiagnosis) {
      return "Признаков патологических изменений тазобедренных суставов не выявлено.";
    }
    ...
    if (leftDiagnosis && rightDiagnosis) {
      ...
      if (sameDiagnosis) {
        ...
        return `Признаки ${diagnosisText} обоих тазобедренных суставов.`;
      } else {
        const leftText = getDiagnosisText(leftDiagnosis);
        const rightText = getDiagnosisText(rightDiagnosis);
        return `Признаки ${rightText} правого тазобедренного сустава; ${leftText} левого тазобедренного сустава.`;
      }
    }
    ...
    if (leftDiagnosis) {
      const diagnosisText = getDiagnosisText(leftDiagnosis);
      return `Признаки ${diagnosisText} левого тазобедренного сустава, правый сустав без особенностей.`;
    }

    // Только правый
    if (rightDiagnosis) {
      const diagnosisText = getDiagnosisText(rightDiagnosis);
      return `Признаки ${diagnosisText} правого тазобедренного сустава, левый сустав без особенностей.`;
    }
  ...
  const getDiagnosisText = (diagnosis) => {
    switch (diagnosis) {
      ...
      case "Коксартроз": return "коксартроза";
      case "Асептический некроз": return "асептического некроза";

...
export default function ElbowJointSurfaceModal({ isOpen, onClose, textareaRef }) {
...
const generateDescriptionCompact = () => {
  ...
  if (!leftOpts.length && !rightOpts.length) {
    return "Суставные поверхности локтевых суставов не изменены, без особенностей.";
  }
  ...
  if (leftOpts.length && rightOpts.length) {
    ...
    if (sameDegree && (!leftPos && !rightPos)) {
      return `Суставные поверхности локтевых суставов ${leftOpts[0]} склерозированы.`;
    }
    ...
    if (sameDegree && samePos) {
      ...
      return `Суставные поверхности локтевых суставов ${leftOpts[0]} склерозированы, преимущественно в ${positionPhrase(leftPos, true)} отделах.`;
    }

    // Если разная степень или позиция
    return `Суставная поверхность правого локтевого сустава ${rightOpts[0]} склерозирована${addPosition(rightPos)}; левого ${leftOpts[0]} склерозирована${addPosition(leftPos)}.`;
  }

  // Если только левый сустав
  if (leftOpts.length) {
    return `Суставная поверхность левого локтевого сустава ${leftOpts[0]} склерозирована${addPosition(leftPos)}, правая без особенностей.`;
  }

  // Если только правый сустав
  if (rightOpts.length) {
    return `Суставная поверхность правого локтевого сустава ${rightOpts[0]} склерозирована${addPosition(rightPos)}, левая без особенностей.`;
  }

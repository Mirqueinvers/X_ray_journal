// src/components/generateDescription/generateDescriptionUniversal.js

export const generateDescriptionUniversal = ({ jointMap, selectedOptions, type = "foot" }) => {
  ...
  // Склеиваем пальцы в диапазоны, например II–IV
  ...
  degreeOrder.forEach((deg) => {
    ...
    Object.entries(sides).forEach(([side, types]) => {
      Object.entries(types).forEach(([typeKey, labels]) => {
        ...
        degreeParts.push(`${deg} сужены в ${fingers} ${jointName} ${sideName} ${limbSuffix}`.trim());
      });
    });
  });

...
export default function ResearchRegionsChart({ startDate, endDate }) {
  ...
  const fetchRegionsData = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE}/api/research-regions`, {
        params: { startDate, endDate }
      });

      if (response.data.success) {
        // Преобразуем данные для круговой диаграммы
        const pieData = response.data.data.map(item => ({
          name: item.region,
          value: item.researchCount
        }));
      ...
      console.error('Ошибка загрузки статистики по регионам:', err);
      setError('Ошибка загрузки данных');
    } finally {
      setLoading(false);
    }
      ...
      tooltip: {
        callbacks: {
          label: (context) => {
            ...
            return `${label}: ${value} исследований (${percentage}%)`;
          }
        },
        titleFont: {
          size: 14,
  ...
  // Общая статистика
  const totalResearch = chartData.reduce((sum, item) => sum + item.value, 0);
    ...
    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-sm text-gray-600">
          <span className="font-medium">{totalResearch}</span> исследований в{' '}
          <span className="font-medium">{totalRegions}</span> регионах
        </div>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <span className="text-gray-600">Топ-3 региона:</span>
            <ul className="mt-1">

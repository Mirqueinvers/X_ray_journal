...
const jointMap = [...rightJoints, ...leftJoints];
  ...
  degreeOrder.forEach(deg => {
    ...
    Object.entries(hands).forEach(([foot, types]) => {
      if (types.Ankle) {
        ankleParts.push(`суставная щель ${footNamesGenitive[foot]} голеностопного сустава ${deg} сужена`);
        delete types.Ankle;
      }
    });
  });
  ...
  degreeOrder.forEach(deg => {
    ...
    if (footParts.length) degreeParts.push(`${deg} сужены в ${footParts.join(", ")}`);
  });

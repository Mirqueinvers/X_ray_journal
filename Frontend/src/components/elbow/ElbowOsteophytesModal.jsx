...
export default function ElbowOsteophytesModal({ isOpen, onClose, textareaRef }) {
  ...
  if (rightLateral && rightMedial && leftLateral && leftMedial) {
    return "Определяются краевые костные разрастания на боковых поверхностях локтевых суставов.";
  }

  // 3️⃣ Если обе стороны только у правого
  if (rightLateral && rightMedial && !leftLateral && !leftMedial) {
    return "Определяются краевые костные разрастания на боковых поверхностях правого локтевого сустава.";
  }

  // 4️⃣ Если обе стороны только у левого
  if (!rightLateral && !rightMedial && leftLateral && leftMedial) {
    return "Определяются краевые костные разрастания на боковых поверхностях левого локтевого сустава.";
  }

  // 🆕 4.1 Если одинаковые стороны на обоих суставах
  if (rightLateral && leftLateral && !rightMedial && !leftMedial) {
    return "Определяются краевые костные разрастания на латеральных поверхностях локтевых суставов.";
  }
  if (rightMedial && leftMedial && !rightLateral && !leftLateral) {
    return "Определяются краевые костные разрастания на медиальных поверхностях локтевых суставов.";
  }
  ...
  if (rightLateral && rightMedial && (leftLateral || leftMedial)) {
    const leftSide =
      leftLateral && leftMedial
        ? "на боковых поверхностях левого локтевого сустава"
        : leftLateral
        ? "на латеральной поверхности левого локтевого сустава"
        : "на медиальной поверхности левого локтевого сустава";
    return `Определяются краевые костные разрастания на боковых поверхностях правого локтевого сустава и ${leftSide}.`;
  }
  ...
  if (leftLateral && leftMedial && (rightLateral || rightMedial)) {
    const rightSide =
      rightLateral && rightMedial
        ? "на боковых поверхностях правого локтевого сустава"
        : rightLateral
        ? "на латеральной поверхности правого локтевого сустава"
        : "на медиальной поверхности правого локтевого сустава";
    return `Определяются краевые костные разрастания ${rightSide} и на боковых поверхностях левого локтевого сустава.`;
  }

  // 7️⃣ Если по одной стороне на каждом суставе (разные комбинации)
  const parts = [];
  if (rightLateral)
    parts.push("на латеральной поверхности правого локтевого сустава");
  if (rightMedial)
    parts.push("на медиальной поверхности правого локтевого сустава");
  if (leftLateral)
    parts.push("на латеральной поверхности левого локтевого сустава");
  if (leftMedial)
    parts.push("на медиальной поверхности левого локтевого сустава");

...
const generateDescriptionCompact = () => {
  ...
  if (!leftOpts.length && !rightOpts.length) {
    return "Суставные щели луче-запястных суставов сохранены, равномерные.";
  }
  ...
  if (leftOpts.length && rightOpts.length) {
    ...
    if (sameDegree && samePos) {
      ...
      if (!leftPos || leftPos === "равномерно") {
        return `Суставные щели луче-запястных суставов ${leftOpts[0]} сужены.`;
      }
      return `Суставные щели луче-запястных суставов ${leftOpts[0]} сужены${addPosition(leftPos, true)}.`;
    } else {
      return `Суставная щель правого луче-запястного сустава ${rightOpts[0]} сужена${addPosition(rightPos)}; левого ${leftOpts[0]} сужена${addPosition(leftPos)}.`;
    }
  }

  // Только левый
  if (leftOpts.length) {
    return `Суставная щель левого луче-запястного сустава ${leftOpts[0]} сужена${addPosition(leftPos)}, правого не изменена.`;
  }

  // Только правый
  if (rightOpts.length) {
    return `Суставная щель правого луче-запястного сустава ${rightOpts[0]} сужена${addPosition(rightPos)}, левого не изменена.`;
  }
